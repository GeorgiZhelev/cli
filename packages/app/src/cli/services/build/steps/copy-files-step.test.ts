...
import {BuildStep, BuildContext, StepResult} from '../build-steps.js'
...
describe('executeCopyFilesStep', () => {
  ...
  describe('manifest_result strategy', () => {
    function makeManifestResult(output: BuildManifestStepOutput): StepResult {
      return {stepId: 'build-manifest', displayName: 'Build Manifest', success: true, duration: 0, output}
    }
    ...
    const step: BuildStep = {
      ...
      type: 'copy_files',
      config: {strategy: 'manifest_result', definition: {}},
    }
    ...
    test('supports a custom stepId via definition config', async () => {
      ...
      const customStep: BuildStep = {
        id: 'copy-static',
        displayName: 'Copy Static Assets',
        type: 'copy_files',
        config: {strategy: 'manifest_result', definition: {stepId: 'custom-manifest-step'}},
      }
      const output: BuildManifestStepOutput = {
        outputFile: '/test/output/build-manifest.json',
        assets: {
      ...
      // When
      const result = await executeCopyFilesStep(customStep, mockContext)
    ...
    test('throws when the referenced step failed', async () => {
      ...
      // When/Then
      await expect(executeCopyFilesStep(step, mockContext)).rejects.toThrow("Step 'build-manifest' didn't succeed")
    })
  })
})
